...
`v0.9.0-beta.9`은 GOG 영문 DOS판용 한국어 패치입니다. 게임 실행 파일 전체나 원작 리소스를 포함하지 않으며, 설치기가 사용자의 적법한 원본 또는 검증된 공개 beta.4·beta.5·beta.6·beta.7·beta.8 설치본을 확인한 뒤 바이너리 델타와 설치 시 생성한 한글 글리프를 적용합니다.
...
공개 beta.4·beta.5·beta.6·beta.7·beta.8 설치자는 제거하지 않고 beta.9으로 직접 업그레이드할 수 있습니다. 기본 이롭게 바탕체는 `INSTALL.cmd`, 사용자 글꼴은 `INSTALL_CUSTOM_FONT.cmd`를 실행합니다. 이전 사용자 글꼴 경로는 저장하지 않으므로 사용자 글꼴 업그레이드에서는 파일을 다시 선택합니다.

설치기는 동봉한 고정 beta.4·beta.5·beta.6·beta.7·beta.8 manifest, 기존 receipt와 현재 설치 파일을 검증합니다. 업그레이드가 실패하면 업그레이드 직전 버전으로 롤백하고, 업그레이드된 beta.9을 제거하면 최초 GOG 원본을 복원합니다.

beta.1~beta.3 설치자는 설치에 사용한 이전 배포 폴더의 `UNINSTALL.cmd`로 원본을 복구한 뒤 beta.9을 설치해야 합니다. 이전 배포 폴더가 없다면 해당 버전 ZIP을 다시 받아 제거할 수 있습니다.
