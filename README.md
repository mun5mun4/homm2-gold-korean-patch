...
- 현재 배포 기준은 `v0.9.0-beta.6`입니다.
- beta.6는 배포 글꼴을 OFL 1.1의 `NanumGothicCoding Regular` 하나로 고정하고 사용자 글꼴 선택 기능을 제거합니다.
- 공개 beta.4와 beta.5 설치본은 beta.6로 직접 업그레이드할 수 있습니다.

배포판은 게임 실행 파일이나 원작 리소스 전체를 포함하지 않고, 사용자가 보유한 정확한 GOG 원본에 바이너리 델타를 적용합니다.

## beta.6 설치와 업그레이드

깨끗한 GOG 원본이나 공개 beta.4·beta.5 설치본에서 beta.6 ZIP의 `INSTALL.cmd`를 실행합니다. 이전 버전에서 어떤 글꼴을 사용했든 beta.6는 동봉한 나눔고딕코딩으로 글리프를 다시 생성하므로, beta.4·beta.5를 먼저 제거하거나 글꼴을 다시 선택할 필요가 없습니다.

beta.1~beta.3은 직접 업그레이드 대상이 아닙니다. 해당 버전 배포 폴더의 `UNINSTALL.cmd`로 GOG 원본을 복구한 뒤 beta.6를 설치하세요. 이전 배포 폴더가 없다면 같은 버전 ZIP을 다시 받아 제거할 수 있습니다.

1. GOG Galaxy, Heroes II, DOSBox를 모두 종료합니다.
2. GitHub Releases에서 `homm2-ko-v0.9.0-beta.6-win-gog.zip`을 받습니다.
...
beta.6는 SIL Open Font License 1.1의 `NanumGothicCoding Regular`에서 설치 시점에 874자 한글 글리프를 생성합니다. 배포 ZIP에 들어가는 글꼴과 설치기가 사용하는 글꼴은 이 파일 하나뿐이며, 사용자 글꼴 선택 명령이나 별도 설치 스크립트는 제공하지 않습니다.
...
직접 업그레이드는 동봉한 고정 beta.4·beta.5 manifest와 기존 receipt를 모두 검증합니다. 중간에 실패하면 업그레이드 직전 버전으로 롤백하고, 업그레이드된 beta.6를 제거하면 이전 베타가 아니라 최초 GOG 원본을 복원합니다.
...
일반 MP2/MX2 지도와 그림에 박힌 버튼·제목은 이번 베타 범위에서 제외했습니다.
...
├─ tools/localization/        최종 교정·캠페인·H2K3 형식 도구
├─ translations/             선별된 번역 원장과 874자 글리프 매핑
├─ packaging/release_assets/ ZIP에 포함되는 문서·스크립트·기본 나눔고딕코딩
├─ tests/                     공개 소스 단위 검사
└─ docs/                      설계 문서와 공개판 활성 파일 해시
```

내부 분석 로그, DOSBox 제어 실험, 캡처, 후보·복구 산출물은 저장소에 포함하지 않습니다. 배포 재포장 방법은 [BUILD_KO.md](BUILD_KO.md), 소스 범위는 [docs/SOURCE_LAYOUT_KO.md](docs/SOURCE_LAYOUT_KO.md)에 설명했습니다.

`docs/ACTIVE_FILE_HASHES.json`은 beta.3 활성 번역 트리를 재포장 입력으로 고정한 source pin입니다. beta.4~beta.6의 공개 결과 해시 목록이 아니므로 내용은 바꾸지 않습니다. 설치 시 생성된 두 AGG의 실제 해시는 로컬 receipt에 기록합니다.
