...
아래 항목은 `v0.9.0-beta.11`의 영상 자막, 기본·사용자 선택 동적 폰트와 공개 beta.4·beta.5·beta.6·beta.7·beta.8·beta.9·beta.10 직접 업그레이드 구성을 설명합니다.

## 배포 도구

- `tools/release/homm2_ko_patcher.py`: 기본·사용자 글꼴 선택, 설치·검증·복구·제거
- `tools/release/homm2_font.py`: 874자 매핑과 선택·대체 face 검사, Pillow 기반 글리프 생성, 원본 팔레트의 버튼 입체 도트와 배경 버튼 동기화, ICN·AGG 보존형 재구성
...
- `tools/localization/h2_video_subtitles.py`: 공개 beta.6 EXE/BIN에서 KSX2 큐·dual-ms 시계·2배 외곽선 렌더러·화면 갱신 클리퍼와 load-base 독립 중계 코드를 포함한 beta.10 EXE/BIN(beta.11에서도 동일)을 재현하고 검증하는 고정 해시 변환
...
`docs/ACTIVE_FILE_HASHES.json`은 beta.3 번역 기반을 고정하는 기존 source pin입니다. beta.4~beta.11 배포 결과 pin이 아니므로 파일 자체는 변경하지 않습니다. beta.11의 EXE/BIN은 release manifest가 고정하고, 설치 시 생성된 AGG 해시는 각 설치의 `_homm2_ko_install/receipt.json`에 기록합니다.
