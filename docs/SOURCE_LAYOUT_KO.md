...
아래 항목은 `v0.9.0-beta.14`의 영상 자막, 기본·사용자 선택 동적 폰트와 공개 beta.4·beta.5·beta.6·beta.7·beta.8·beta.9·beta.10·beta.11·beta.12·beta.13 직접 업그레이드 구성을 설명합니다.

## 배포 도구

- `tools/release/homm2_ko_patcher.py`: 기본·사용자 글꼴 선택, 설치·검증·복구·제거 및 과거 v3 설치 기록 호환
- `tools/release/homm2_font.py`: 874자 매핑과 선택·대체 face 검사, Pillow 기반 글리프 생성, 원본 팔레트의 버튼 입체 도트·실제 평면 중앙 배치·테두리 복원과 배경 버튼 동기화, ICN·AGG 보존형 재구성
- `tools/release/build_release.py`: 검증된 원본과 활성 패치 트리에서 manifest v2 배포 디렉터리 생성
- `tools/release/approved_main_menu.py`: 빌드 시 메인 메뉴 델타 재현과 원본·결과·편집 범위·팔레트 검증
...
- `translations/interface/main_menu/`: 승인된 메인 메뉴 3개 리소스의 원본 의존 BSDIFF40 델타와 source/target 해시
...
- `tools/localization/h2_video_subtitles.py`: 공개 beta.6 EXE/BIN에서 KSX2 큐·dual-ms 시계·2배 외곽선 렌더러·화면 갱신 클리퍼와 load-base 독립 중계 코드를 포함한 beta.10 EXE/BIN(beta.14에서도 동일)을 재현하고 검증하는 고정 해시 변환
...
- `packaging/release_assets/upgrades/v0.9.0-beta.10-manifest.json`: 공개 beta.10 직접 업그레이드 검증용 고정 manifest
- `packaging/release_assets/upgrades/v0.9.0-beta.11-manifest.json`: 공개 beta.11 직접 업그레이드 검증용 고정 manifest
- `packaging/release_assets/upgrades/v0.9.0-beta.12-manifest.json`: 공개 beta.12 직접 업그레이드 검증용 고정 manifest
- `packaging/release_assets/upgrades/v0.9.0-beta.13-manifest.json`: 공개 beta.13 직접 업그레이드 검증용 고정 manifest
...
`HEROES2.AGG`의 폰트 없는 기반은 번역된 BIN 8개와 승인 메뉴 `BTNSHNGL.ICN`·`HEROES.ICN`을 유지합니다. `HEROES2X.AGG` 기반은 승인된 확장 `HEROES.ICN`만 원본과 다릅니다. 메뉴는 저장소의 리소스별 델타에서 재현하고, `HEROWIND.BIN`의 고정 `Knowledge` 슬롯은 `지력`으로 교정합니다. 일반 글리프와 한글 버튼·`병력당 비용:`은 설치 시 선택 글꼴로 생성합니다. 메인 메뉴 전용 그래픽은 그 과정에서 다시 그리지 않습니다.
...
중앙 배치 교정은 132×62 큰 메뉴 89개 상태와 `RECRUIT.ICN:4/5` 2개 상태에 한정합니다. 총 16개 리소스(기본 13개·확장 3개)를 교정하고 나머지 163개 버튼 상태, 글리프·색조·EXE·문자열 은행은 beta.11과 같게 유지합니다. 생성된 AGG와 캡처 대신 재현 코드와 회귀검사를 공개합니다.

## 해시 문서의 범위

`docs/ACTIVE_FILE_HASHES.json`은 beta.3 번역 기반을 고정하는 기존 source pin입니다. beta.4~beta.14 배포 결과 pin이 아니므로 파일 자체는 변경하지 않습니다. beta.14의 EXE/BIN은 release manifest가 고정하고, 설치 시 생성된 AGG 해시는 각 설치의 `_homm2_ko_install/receipt.json`에 기록합니다.
