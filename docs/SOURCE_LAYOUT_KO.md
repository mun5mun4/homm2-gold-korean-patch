...
아래 항목은 `v0.9.0-beta.8`의 영상 자막, 기본·사용자 선택 동적 폰트와 공개 beta.4·beta.5·beta.6·beta.7 직접 업그레이드 구성을 설명합니다.
...
- `tools/localization/h2_video_subtitles.py`: 공개 beta.6 EXE/BIN에서 KSX2 큐·dual-ms 시계·2배 외곽선 렌더러·화면 갱신 게이트를 포함한 beta.7 EXE/BIN을 재현하고 검증하는 고정 해시 변환
...
## beta.8 글꼴·라이선스·업그레이드 자산

- `packaging/release_assets/fonts/NanumGothicCoding-Regular.ttf`: 배포판에 유일하게 동봉하는 기본·대체 글꼴 NanumGothicCoding Regular(OFL 1.1)
...
`packaging/release_assets/INSTALL_CUSTOM_FONT.cmd`와 설치기의 `--choose-font`, `--font-file`, `--font-index`는 사용자가 보유한 로컬 TTF·OTF·TTC·OTC를 설치 시 선택하는 진입점입니다. 선택 파일은 저장소·배포 자산이나 게임 폴더에 복사하지 않습니다. 배포 ZIP에 들어가는 글꼴 파일은 기본·대체용 나눔고딕코딩 하나뿐입니다.

`Iropke Batang Medium`은 v3 bearing을 확인하기 위해 로컬에서 선택한 수동 검증 예시입니다. 해당 글꼴, 라이선스 파일, 후보 AGG·미리 생성한 래스터는 공개 소스나 배포 자산에 포함하지 않습니다.
...
`docs/ACTIVE_FILE_HASHES.json`은 beta.3 번역 기반을 고정하는 기존 source pin입니다. beta.4~beta.8 배포 결과 pin이 아니므로 파일 자체는 변경하지 않습니다. beta.8의 EXE/BIN은 release manifest가 고정하고, 설치 시 생성된 AGG 해시는 각 설치의 `_homm2_ko_install/receipt.json`에 기록합니다.
