...
아래 항목은 `v0.9.0-beta.7`의 영상 자막, 나눔고딕코딩 고정 동적 폰트와 공개 beta.4·beta.5·beta.6 직접 업그레이드 구성을 설명합니다.
...
`docs/ACTIVE_FILE_HASHES.json`은 beta.3 번역 기반을 고정하는 기존 source pin입니다. beta.4~beta.7 배포 결과 pin이 아니므로 파일 자체는 변경하지 않습니다. beta.7의 EXE/BIN은 release manifest가 고정하고, 설치 시 생성된 AGG 해시는 각 설치의 `_homm2_ko_install/receipt.json`에 기록합니다.
