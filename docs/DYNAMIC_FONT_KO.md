...
오리지널 `DATA/HEROES2.AGG`에는 모집 창 배경 `RECRBKG.ICN`이 있습니다. beta.12 설치기는 먼저 순정 payload가 91,987바이트, SHA-256 `D7B9EF7C819CADACFABF0BCB857976535945DC6F52DC60581D30AC69513E7024`인지 확인합니다. 정확히 일치할 때만 sprite 0의 ROI `(157, 51, 96, 17)`을 같은 행의 안전한 배경으로 지우고, 생성된 `SMALFONT.ICN` 글리프로 `병력당 비용:`을 가운데 배치합니다.
...
장식 메인 메뉴는 beta.13부터 선택 글꼴에서 다시 그리지 않고 승인된 전용 그래픽을 보존합니다. `BTNSHNGL.ICN`의 20개 상태와 기본·확장 `HEROES.ICN` 2종을 정확한 원본에 리소스별 BSDIFF40 델타로 적용합니다. 이 3개 결과는 폰트 없는 AGG 기반에 들어가며 `rebuild_agg_fonts()`는 그대로 유지합니다. 기본·사용자 글꼴에서 같은 메뉴가 나와야 합니다. [메인 메뉴 설계](MAIN_MENU_KO.md)에 source/target 검증과 팔레트 보존 범위를 설명했습니다.

beta.11에서 도입한 56개 문구·254개 버튼 상태의 스타일을 유지합니다. 입체 글씨를 쓰는 249개 상태에는 재질별 명암을 적용합니다. 갈색·회색·금색 버튼에 맞춘 짙은 본체와 밝은 왼쪽 아래 1픽셀 가장자리를 원본 256색 팔레트에서 선택하며, 눌린 상태는 별도의 어두운 색조를 사용합니다. 원본이 평면인 확장 캠페인 이름 5개는 같은 표현을 유지합니다. 41개 배경 복사 위치도 재생성한 버튼과 동기화합니다.

새 효과는 생성된 글리프의 전경 모양과 배치를 재사용하며 기존 오른쪽 아래 그림자를 정리합니다. 버튼의 모서리 반사광은 원본의 작은 연결 성분으로 한정해 복원합니다. 큰 메뉴 버튼 오른쪽, `BTNNEWGM.ICN:4` 위쪽, `OVERVIEW.ICN:2/3` 오른쪽에 남았던 영문 획은 별도 선언된 작은 정리 영역에서 원본과 일치하는 픽셀만 제거합니다. 이 영역까지 포함한 허용 mask 밖 픽셀과 transform, sprite 크기·offset은 보존합니다.

## beta.12 버튼 평면 중앙 배치

132×62 큰 메뉴의 실제 평면은 일반 상태 y=4..51, 눌린 상태 y=5..52이며 중심은 각각 27.5와 28.5입니다. 글리프 전경과 왼쪽 아래 입체 가장자리를 합친 보이는 글씨 범위를 이 중심에 맞춥니다. 기존 글리프 자체의 bearing·셀·팔레트는 바꾸지 않습니다. 평면 스타일인 확장 캠페인 이름도 평면 표현을 유지한 채 같은 위치 규칙을 적용합니다.

기본 글꼴 결과에서는 큰 메뉴 89개 상태가 beta.11보다 3픽셀 위로 이동합니다. `RECRUIT.ICN:4/5`의 `최대`는 일반 상태 평면 y=4..20(중심 12)과 눌린 상태 y=5..21(중심 13)을 기준으로 각각 3·4픽셀 올립니다. 종전 글씨·그림자가 침범한 아래쪽 경사 테두리는 순정 픽셀로 복원하고, 새 글씨는 평면 안에 배치합니다.

총 91개 상태·16개 리소스(기본 13개·확장 3개)가 바뀝니다. 나머지 163개 버튼 상태, 글리프 모양과 색조, EXE·문자열 은행은 beta.11과 같습니다. 기본·사용자 글꼴은 각각의 실제 보이는 글씨 범위로 배치하며 사용자 글꼴에 기본 글꼴 해시를 강제하지 않습니다. beta.12의 중앙 배치와 보존 범위는 소스 회귀검사와 격리된 fixture에서 확인했습니다.
...
배포 빌더는 활성 AGG를 그대로 델타로 만들지 않습니다. GOG 원본을 기준으로 번역 BIN과 승인된 메인 메뉴만 반영한 폰트 없는 기반을 만들고, 일반 글리프와 버튼은 설치 시 생성합니다.

### `DATA/HEROES2.AGG`

GOG 원본을 기준으로 다음 번역 BIN 8개와 승인 메뉴 `BTNSHNGL.ICN`·`HEROES.ICN`을 유지합니다.
...
`HEROWIND.BIN`은 payload offset 303의 `0A 00` 길이 word를 보존하면서 offset 305의 10바이트 `Knowledge\0`만 `82 D8 82 95 00 00 00 00 00 00`(`지력`)으로 바꿉니다. 이미 교정된 슬롯은 그대로 허용하고 예상 밖의 값이면 배포 빌드를 중단합니다. 이 BIN의 나머지 payload는 변경하지 않습니다.

그 밖의 AGG 기반 리소스는 원본을 사용합니다. `FONT.ICN`, `SMALFONT.ICN`, 일반 버튼과 `RECRBKG.ICN:0` 모집 비용 명패는 설치 시 선택 글꼴로 생성합니다. 메인 메뉴는 검증된 리소스별 델타에서 재현한 전용 그래픽을 유지합니다.

### `DATA/HEROES2X.AGG`

폰트 없는 기반은 GOG 원본 `HEROES2X.AGG`에서 `HEROES.ICN`만 승인된 확장판 메뉴로 교체합니다. 설치 단계에서 새 `FONT.ICN`, `SMALFONT.ICN`과 일반 버튼 글씨를 넣습니다.
