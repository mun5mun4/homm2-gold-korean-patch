...
이 문서는 검증된 GOG 원본 트리와 현재 활성 한국어 패치 트리에서 `v0.9.0-beta.8` 패키지를 만드는 절차입니다. 공개 beta.4·beta.5·beta.6·beta.7을 직접 업그레이드 입력으로 지원합니다.

beta.8은 48개 고정 BSDIFF40 결과와 설치 시 선택 글꼴로 생성하는 2개 AGG를 manifest v2로 묶고, 직접 업그레이드 검증용으로 공개 beta.4·beta.5·beta.6·beta.7 manifest를 고정해 포함합니다. 완성 AGG, 미리 생성한 한글 래스터와 사용자 선택 글꼴 파일은 배포하지 않습니다. 배포 ZIP에는 기본·대체용 나눔고딕코딩과 로컬 파일 선택 진입점만 포함합니다.
...
- 수정되지 않은 지원 GOG 영문판
- beta.3 번역 기반에 검증된 beta.7 영상 자막 EXE/BIN과 beta.8 동적 UI 입력을 적용한 활성 패치 트리
...
`docs/ACTIVE_FILE_HASHES.json`은 `v0.9.0-beta.3` 번역 기반을 고정한 기존 source pin입니다. beta.8은 이 기반의 캠페인·AGG 현지화에 beta.7에서 검증된 영상 자막 EXE/BIN과 새 동적 UI 계약을 추가합니다.

이 파일은 beta.4~beta.8 출력 해시 목록이 아니므로 `ACTIVE_FILE_HASHES.json` 자체를 갱신하지 않습니다. beta.8의 최종 EXE/BIN은 release manifest가 고정하며, 설치 시 생성된 두 AGG의 실제 해시는 사용자 PC의 receipt가 보존합니다.
...
beta.8은 beta.7에서 사용자 수동 시험을 통과한 다음 파일 쌍을 그대로 유지합니다. 재포장에 사용하는 활성 트리도 이 identity와 정확히 일치해야 합니다.
...
beta.3의 고정 바탕체 `FONT.ICN`·`SMALFONT.ICN`과 시험 이미지 UI는 기반에서 제거됩니다. 설치 시 기본 나눔고딕코딩 또는 사용자가 선택한 로컬 글꼴에서 새 `FONT.ICN`·`SMALFONT.ICN`과 한글 버튼 글씨를 생성해 두 AGG에 넣습니다. 선택 글꼴에 없는 매핑 문자는 나눔고딕코딩으로 보완합니다. 메인 메뉴 배경·장식은 원본을 유지하고 선언된 버튼 글씨 ROI만 바꿉니다. 오리지널 `HEROES2.AGG`에서는 순정 `RECRBKG.ICN` identity를 먼저 확인한 뒤 sprite 0의 고정 ROI에 같은 작은 글꼴로 `병력당 비용:`을 생성합니다. 자세한 배열은 [docs/DYNAMIC_FONT_KO.md](docs/DYNAMIC_FONT_KO.md)를 확인하세요.
...
개발 중에는 사용자가 로컬로 보유한 `Iropke Batang Medium`을 선택한 별도 후보에서 874자, 공통 기준선과 전경 clip 0을 확인했습니다. 이 글꼴 파일과 그 글꼴로 생성한 래스터·AGG는 빌드 입력이나 배포 자산이 아닙니다.
...
python tools/release/build_release.py ^
  ...
  --patched-root "C:\path\to\beta8-active-korean-tree" ^
  --patcher-exe "dist\homm2-ko-patcher.exe" ^
  --output "release_output\homm2-ko-v0.9.0-beta.8" ^
...
- 874자 매핑, 기본·대체용 `NanumGothicCoding-Regular.ttf`, `INSTALL_CUSTOM_FONT.cmd`, 동적 폰트 빌더와 OFL 1.1 고지
- 공개 beta.4·beta.5·beta.6·beta.7의 고정 manifest `upgrades/v0.9.0-beta.4-manifest.json`, `upgrades/v0.9.0-beta.5-manifest.json`, `upgrades/v0.9.0-beta.6-manifest.json`, `upgrades/v0.9.0-beta.7-manifest.json`
...
python tools/release/package_release.py ^
  --release-dir "release_output\homm2-ko-v0.9.0-beta.8" ^
  --output-dir "release_output\github-assets-v0.9.0-beta.8" ^
  --version "v0.9.0-beta.8"
```
...
7. 오리지널 모집 창의 `Cost per troop:`이 `병력당 비용:`으로 바뀌고 지정 ROI 밖 픽셀·transform과 다른 AGG 엔트리가 보존되는지 확인
8. 메인 메뉴 배경·장식과 비대상 ROI가 원본과 같고 선언된 버튼 글씨만 선택 글꼴로 바뀌는지 확인

beta.1~beta.3 직접 업그레이드는 지원하지 않으며 이전 버전을 제거한 GOG 원본에서 시험합니다. 네 upgrade manifest는 크기와 SHA-256이 고정된 공개 beta.4·beta.5·beta.6·beta.7 manifest와 정확히 일치해야 합니다. 최종 ZIP에는 `INSTALL_CUSTOM_FONT.cmd`가 반드시 있어야 하지만, 글꼴 파일은 `NanumGothicCoding-Regular.ttf` 하나만 허용합니다. `Iropke Batang Medium`을 포함한 다른 TTF·OTF·TTC·OTC나 미리 생성한 래스터·AGG를 임의로 추가하면 fail-closed 패키징이 중단되어야 합니다.

이 도구는 번역 개발 전 과정을 GOG 원본부터 반복하는 통합 현지화 빌더가 아니라, 검증된 활성 번역 트리를 beta.8 배포 형식으로 재포장하는 도구입니다.
