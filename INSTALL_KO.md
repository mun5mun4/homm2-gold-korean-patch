...
이 문서는 `v0.9.0-beta.8` 기준입니다. 공개 beta.4·beta.5·beta.6·beta.7 설치본은 직접 업그레이드할 수 있고, beta.1~beta.3은 먼저 제거해야 합니다.
...
- beta.4·beta.5·beta.6·beta.7이 설치돼 있다면 제거하지 말고 아래의 직접 업그레이드 절차를 사용합니다.

이전 beta.1~beta.3 배포 폴더가 없다면 같은 버전의 ZIP을 GitHub Releases에서 다시 받아 제거할 수 있습니다. 설치기는 지원 GOG 원본 또는 검증된 공개 beta.4·beta.5·beta.6·beta.7 설치본 외의 수정 파일에는 새 델타나 글리프를 겹쳐 적용하지 않습니다.

설치기는 `GAMES`, `*.HS`, `HEROES2.CFG`, 일반 지도와 개인 세이브를 건드리지 않습니다.

## 설치

beta.8 ZIP을 모두 푼 뒤 기본 나눔고딕코딩을 쓰려면 `INSTALL.cmd`를 실행합니다. 사용자가 보유한 로컬 글꼴을 쓰려면 `INSTALL_CUSTOM_FONT.cmd`를 실행하고 TTF·OTF·TTC·OTC 파일을 선택합니다. 선택 글꼴에 없는 매핑 문자는 SIL Open Font License 1.1로 동봉한 `fonts/NanumGothicCoding-Regular.ttf`가 자동으로 보완합니다.

선택한 글꼴 파일은 설치 시 이 PC에서만 읽고 패치 폴더로 복사하거나 수집·업로드·배포하지 않습니다. 설치기는 글꼴 이름이나 라이선스로 선택을 막지 않으므로 해당 파일을 사용할 권한은 사용자가 확인해야 합니다. 설치 후 글꼴만 바꾸려면 `UNINSTALL.cmd`로 원본을 복구한 뒤 원하는 설치 방식을 다시 실행합니다.

개발 검증에서는 로컬의 `Iropke Batang Medium`을 사용자 선택 예시로 사용했지만, 이 파일이나 이 글꼴로 만든 래스터·AGG를 배포하지 않습니다. 사용자는 자신이 사용할 권한이 있는 글꼴 파일을 직접 선택해야 합니다.
...
## beta.4·beta.5·beta.6·beta.7에서 직접 업그레이드

공개 beta.4·beta.5·beta.6·beta.7 중 하나가 설치돼 있다면 기본 글꼴로 바꾸거나 유지할 때 `INSTALL.cmd`, 사용자 글꼴로 만들 때 `INSTALL_CUSTOM_FONT.cmd`를 실행합니다. 이전 receipt에는 사용자 글꼴 경로가 저장되지 않으므로 사용자 글꼴 업그레이드에서는 파일을 다시 선택해야 합니다.

설치기는 배포 ZIP에 고정해 둔 beta.4·beta.5·beta.6·beta.7 manifest의 크기·SHA-256과 기존 receipt·현재 설치 파일을 함께 확인합니다. 모두 맞을 때만 최초 GOG 원본 백업에서 beta.8 후보를 만들고, 현재 설치 파일을 별도 백업한 뒤 교체합니다. 실패하면 업그레이드 직전 버전으로 롤백합니다. 업그레이드된 beta.8의 `UNINSTALL.cmd`는 이전 베타가 아니라 최초 GOG 원본을 복원합니다.

## beta.8 설치 과정

1. GOG gameId·buildId·언어와 원본 50개 해시 확인
2. manifest v2, 고정 beta.4·beta.5·beta.6·beta.7 upgrade manifest, 48개 고정 BSDIFF40, 2개 동적 AGG 기반 델타, `KOREAN.BIN`, 874자 매핑과 기본 나눔고딕코딩 해시 확인
...
- `지원하지 않는 직접 upgrade 버전입니다`: beta.1~beta.3을 제거해 GOG 원본으로 돌아간 뒤 설치하세요. beta.4·beta.5·beta.6·beta.7은 공개 배포판과 설치 상태가 정확히 일치하는지 확인하세요.
...
- `글꼴 파일을 읽을 수 없습니다`: 기본 설치라면 ZIP을 다시 받아 모두 푼 뒤 `fonts/NanumGothicCoding-Regular.ttf`가 있는지 확인하고, 사용자 글꼴 설치라면 선택한 파일이 지원되는 TTF·OTF·TTC·OTC인지 확인하세요.
